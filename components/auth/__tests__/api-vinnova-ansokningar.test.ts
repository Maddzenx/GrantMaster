 ...
 */
import { ansokningarHandler } from '../../../pages/api/vinnova/ansokningar';
import { testApiHandler } from '../../../lib/testUtils';
import { expect } from '@jest/globals';

describe('/api/vinnova/ansokningar', () => {
  let mockGetAnsokningar: jest.Mock;
  ...
  const fakeResults = [{ Diarienummer: '123', Titel: 'T', Status: 'S', Beslutsdatum: '2020-01-01', foo: 'bar' }];
  const fakeData = { results: fakeResults, meta: { total: 1 } };
  const sanitizedResults = [{ Diarienummer: '123', Titel: 'T', Status: 'S', Beslutsdatum: '2020-01-01' }];
  const fakeCacheKey = 'ansokningar:' + JSON.stringify(fakeQuery);

  beforeEach(() => {
    mockGetAnsokningar = jest.fn().mockResolvedValue(fakeData);
  ...
  it('handles OPTIONS method', async () => {
    const { res }: any = await testApiHandler((req, res) => ansokningarHandler(req, res, {
  ...
  it('returns 200 and sanitized data on success', async () => {
    const { res }: any = await testApiHandler((req, res) => ansokningarHandler(req, res, {
      ...
      requireUser: mockRequireUser,
      validate: mockValidate,
      getAnsokningar: mockGetAnsokningar,
